...
async def fetch_html(session: ClientSession, url: str) -> BeautifulSoup:
    """Fetch HTML with retry logic and exponential backoff for rate limiting."""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if e.status == 503 and attempt < MAX_RETRIES - 1:
                ...
                print(f"  ⚠ Rate limited (503), retrying in {delay}s (attempt {attempt + 1}/{MAX_RETRIES})")
                await asyncio.sleep(delay)
            else:
                # Either not a 503 or we've exhausted retries
                raise
